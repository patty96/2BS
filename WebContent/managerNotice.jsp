    ...
    <section>
        <div class="content">
		    <div class="noticeWrap">
		        <h1>
		        	공지사항
			        <a href="ManagerController?type=getNoticeWriter" class="write">글쓰기</a>
	        	</h1>
		        <table class="notice">
		            <colgroup>
		                <col width="10%"/>
		            ...
		            <tbody>
		            	<c:forEach var="importNotice" items="${importList }">
			                <tr class="importNotice">
			                    ...
			                    <td>
				                    <a href="ManagerController?type=detailNotice&screenType=1&rNum=${importNotice.r_num }&notNum=${importNotice.not_num }" class="noticeTitle">${importNotice.not_name }</a>
			                    </td>
		            	...
		            	<c:forEach var="notice" items="${list }">
			                <tr>
			                    ...
			                    <td>${notice.r_num }</td>
			                    <td>
				                    <a href="ManagerController?type=detailNotice&screenType=1&rNum=${notice.r_num }&notNum=${notice.not_num }" class="noticeTitle">${notice.not_name }</a>
			                    </td>
		        ...
		        <div class="searchWrap manager">
		        	<form class="searchForm" method="post">
			        	<select name="idx">
				       	...
				       	<input type="button" onclick="searchGoM(this.form);">  
		        	</form>
		        </div>		        
		        <div class="paging">
					<ol>
						<c:choose>
							...
							<c:otherwise>
								<li class="prev">
									<a href="ManagerController?type=getNotice&screenType=1&cPage=${pvo.beginPage-1}"><img src="img/left_arrow.png" alt="이전"></a>
								</li>
							</c:otherwise>
						</c:choose>
						...
						<c:forEach var="pageNo" begin="${pvo.beginPage}" end="${pvo.endPage}">
							<c:choose>
								...
								<c:otherwise>
									<li>
										<a href="ManagerController?type=getNotice&screenType=1&cPage=${pageNo}">${pageNo}</a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
							...
							<c:otherwise>
								<li class="next">
									<a href="ManagerController?type=getNotice&screenType=1&cPage=${pvo.endPage + 1}"><img src="img/right_arrow.png" alt="다음"></a>
								</li>
							</c:otherwise>
						</c:choose>
					</ol>
